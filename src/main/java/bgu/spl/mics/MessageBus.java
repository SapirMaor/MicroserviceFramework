...
public interface MessageBus {
     ...
     * @pre: None
     * @post: isMicroServiceSubscribedEvent(type, m) == true
     */
     ...
     * @param m    	The subscribing micro-service.
     * @pre: None
     * @post: isMicroServiceSubscribedBroadcast(type, m) == true
     */
     ...
     * @pre: @pre(e.Future.get()) == null)
     * @post: e.Future.get()) == result
     */
     ...
     * @pre: None
     * @post: for each micro service: if(isMicroServiceSubscribedBroadcast(b.getClass, m) == true) containsBroadcast(b,m) == true;
     * b is in each of the MicroService's, subscribed to b.getClass, queues
     */
     ...
     * @pre: None
     * @post: there is only one microService m, that:
     * isMicroServiceSubscribedEvent(e.getClass, m) == true && containsEvent(e,m) == true;
     * e is in only one of the MicroService's, subscribed to Event<T>, queue
     */
     ...
     *                              to became available.
     * @pre: if(isMicroServiceRegistered(m) {containsEvent(awaitMessage(m),m) || containsBroadcast(awaitMessage(m),m)}
     * @post: if(isMicroServiceRegistered(m) {!containsEvent(awaitMessage(m),m) && !containsBroadcast(awaitMessage(m),m)}
     */
    Message awaitMessage(MicroService m) throws InterruptedException;

    <T> Boolean isMicroServiceSubscribedEvent(Class<? extends Event<T>> type, MicroService m);
    Boolean isMicroServiceSubscribedBroadcast(Class<? extends Broadcast> type, MicroService m);
